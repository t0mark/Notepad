...
class ClockPublisher:
    def __init__(self):
        rospy.init_node('manual_clock_publisher_points')
        self.pub = rospy.Publisher('/clock_points', Clock, queue_size=10)
        self.rate = rospy.Rate(100)  # 100 Hz
        
        # bag에서 시간 동기화를 위해 센서 데이터 구독
        self.sub = rospy.Subscriber('/ouster/points', PointCloud2, self.sensor_callback)
