...
int ball_query_with_count_wrapper_stack(int B, int M, float radius, int nsample,
    at::Tensor new_xyz_tensor, at::Tensor new_label_tensor, at::Tensor new_xyz_batch_cnt_tensor,
    at::Tensor xyz_tensor, at::Tensor label_tensor, at::Tensor xyz_batch_cnt_tensor, 
    at::Tensor count_tensor) {
    CHECK_INPUT(new_xyz_tensor);
    CHECK_INPUT(xyz_tensor);

    CHECK_INPUT(new_label_tensor);
    CHECK_INPUT(label_tensor);
    ...
    const float *new_label = new_label_tensor.data<float>();
    ...
    ball_query_with_count_kernel_launcher_stack(B, M, radius, nsample, new_xyz, new_label, new_xyz_batch_cnt, xyz, label, xyz_batch_cnt, count);
    return 1;
}