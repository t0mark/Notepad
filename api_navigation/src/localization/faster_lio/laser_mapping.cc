...
#include "localization/faster_lio/laser_mapping.h"
#include "localization/faster_lio/utils.h"
...
void LaserMapping::SubAndPubToROS(ros::NodeHandle &nh) {
    ...
    bool use_standard_lidar_sub = true;
#ifdef FASTER_LIO_HAS_LIVOX
    if (preprocess_->GetLidarType() == LidarType::AVIA) {
        ...
        use_standard_lidar_sub = false;
    }
#endif
    if (use_standard_lidar_sub) {
#ifndef FASTER_LIO_HAS_LIVOX
        if (preprocess_->GetLidarType() == LidarType::AVIA) {
            LOG(WARNING) << "Livox 지원이 비활성화되어 sensor_msgs::PointCloud2 구독자로 대체합니다.";
        }
#endif
