# API Navigation (Faster-LIO Core)

[![license](https://img.shields.io/badge/License-BSD-brightgreen.svg)](LICENSE)

**api_navigation** integrates Faster-LIO perception, INS fusion, Kakao Mapping, and DWA planning into a single ROS package. At its core it still uses the lightweight, tightly-coupled LiDAR-inertial odometry system based on [Fast-LIO2](https://github.com/hku-mars/FAST_LIO). Faster-LIO achieves a 1.5-2x speed improvement over Fast-LIO2 by leveraging parallel sparse incremental voxels (iVox), and the surrounding launch/config files extend it with INS + planner utilities.
...
2.  **Launch Faster-LIO Node** (in a new terminal):
    - **Livox Avia**:
      ```bash
      roslaunch api_navigation launch/perception/mapping_avia.launch
      ```
    - **Ouster OS1-32**:
      ```bash
      roslaunch api_navigation launch/perception/mapping_ouster32.launch
      ```
    - **Velodyne VLP-16**:
      ```bash
      roslaunch api_navigation launch/perception/mapping_velodyne.launch
      ```
...
**Command Format**:
```bash
./devel/lib/api_navigation/run_mapping_offline --bag_file <path_to_bag> --config_file <path_to_config>
```

**Example**:
```bash
# Process data from an Ouster 32-line LiDAR
./devel/lib/api_navigation/run_mapping_offline \
